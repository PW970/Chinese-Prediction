import { getHourIndex, getShiChenName } from './timeHelper.js';
...
const MONTH_STEM_START = { 甲: '丙', 己: '丙', 乙: '戊', 庚: '戊', 丙: '庚', 辛: '庚', 丁: '壬', 壬: '壬', 戊: '甲', 癸: '甲' };
const HOUR_STEM_START = { 甲: '甲', 己: '甲', 乙: '丙', 庚: '丙', 丙: '戊', 辛: '戊', 丁: '庚', 壬: '庚', 戊: '壬', 癸: '壬' };
...
function parseGanzhiPair(pair) {
  return { stem: pair[0], branch: pair[1] };
}

function getYearPillar(lunarData) {
  return { label: lunarData.yearName, ...parseGanzhiPair(lunarData.yearName) };
}

function getMonthPillar(lunarData) {
  const yearStem = getYearPillar(lunarData).stem;
  const firstStem = MONTH_STEM_START[yearStem];
  const stemIndex = mod(STEMS.indexOf(firstStem) + lunarData.month - 1, 10);
...
function getDayPillar(date) {
  ...
  const label = ganzhiFromIndex(mod(diffDays, 60));
  return { label, ...parseGanzhiPair(label) };
}

function getHourPillar(date, dayStem) {
  const firstStem = HOUR_STEM_START[dayStem];
  const hourIndex = getHourIndex(date.getHours());
  const stemIndex = mod(STEMS.indexOf(firstStem) + hourIndex - 1, 10);
  const branch = BRANCHES[mod(hourIndex - 1, 12)];
  return { label: `${STEMS[stemIndex]}${branch}`, stem: STEMS[stemIndex], branch };
}

function summarizeFiveElements(pillars) {
  const summary = { 木: 0, 火: 0, 土: 0, 金: 0, 水: 0 };
  pillars.forEach((pillar) => {
    summary[ELEMENTS[pillar.stem]] += 1;
    summary[ELEMENTS[pillar.branch]] += 1;
  });
  return summary;
}

function computeBazi(date, lunarData) {
  const year = getYearPillar(lunarData);
  const month = getMonthPillar(lunarData);
  const day = getDayPillar(date);
  const hour = getHourPillar(date, day.stem);
  const pillars = [year, month, day, hour];
  return { pillars, fiveElements: summarizeFiveElements(pillars), shichen: getShiChenName(date.getHours()) };
}

function buildQimenChart(lunarData) {
...
function buildZiweiChart(lunarData) {
...
export { buildQimenChart, buildZiweiChart, computeBazi };
